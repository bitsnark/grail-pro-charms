...
describe('pegout e2e test', () => {
    let app: any;
    let peginResult: any;
    let btcUserPayment: any;
    let charmsUserPayment: any;
    ...
    beforeAll(async () => {
        ...
        // Create charms user payment for pegout
        const charmsAmount = 500000;
        charmsUserPayment = await userPaymentCli([
            '--app-id', app.appId,
            '--app-vk', app.appVk,
            '--type', 'charms',
            '--current-public-keys', DEPLOYER.publicKey,
    ...
    it('should execute pegout successfully', async () => {
        await mintBlock();

        const pegoutAmount = 300000;

        const pegoutResult = await pegoutCli([
            '--app-id', app.appId,
            '--app-vk', app.appVk,
            '--previous-nft-txid', peginResult[1], // spell txid from pegin
            '--new-public-keys', DEPLOYER.publicKey,
            '--new-threshold', '1',
            '--private-keys', DEPLOYER.privateKey,
            '--user-payment-txid', charmsUserPayment.txid,
            '--recovery-public-key', charmsUserPayment.recoveryPublicKey,
            '--amount', pegoutAmount.toString(),
            ...TEST_CLI_ARGS,
        ]);

        expect(pegoutResult).toBeTruthy();
        expect(pegoutResult).toHaveLength(2);
        expect(pegoutResult[0]).toBeTruthy(); // funding txid
        expect(pegoutResult[1]).toBeTruthy(); // spell txid
    });

    it('should execute pegout with multiple cosigners', async () => {
        await mintBlock();
        ...
        // Create charms user payment with new cosigners
        const charmsAmount = 400000;
        const newCharmsUserPayment = await userPaymentCli([
            '--app-id', app.appId,
            '--app-vk', app.appVk,
            '--type', 'charms',
        ...
        const pegoutResult = await pegoutCli([
            ...
            '--new-threshold', '1',
            '--private-keys', `${DEPLOYER.privateKey},${COSIGNER_1.privateKey},${COSIGNER_2.privateKey}`,
            '--user-payment-txid', newCharmsUserPayment.txid,
            '--recovery-public-key', newCharmsUserPayment.recoveryPublicKey,
            '--amount', pegoutAmount.toString(),
            ...TEST_CLI_ARGS,
        ]);
    ...
    it('should fail if user payment is not found', async () => {
        await mintBlock();
        ...
        try {
            await pegoutCli([
                '--app-id', app.appId,
                '--app-vk', app.appVk,
                '--previous-nft-txid', peginResult[1],
                '--new-public-keys', DEPLOYER.publicKey,
                '--new-threshold', '1',
                '--private-keys', DEPLOYER.privateKey,
