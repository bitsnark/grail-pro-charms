...
function getSpellActions(spell) {
    const sa = [];
...
async function dot(context, transactionMap, out = console) {
    ...
    Object.values(transactions).forEach(txinfo => {
        txinfo.tx.ins.forEach((_, index) => {
            out.log(`"input-${txinfo.txid}:${index}" [rank="min", shape="point" label=""]`);
            out.log(`"input-${txinfo.txid}:${index}" -> "${txinfo.txid}" [style="${txinfo.spell && index == txinfo.tx.ins.length - 1 ? 'dotted' : 'solid'}", rank="source", tailport="s", weight=1000000]`);
        });
        if (txinfo.spell) {
        ...
        txinfo.tx.outs.forEach((output, index) => {
            ...
            if (txinfo.spell && output.value == 1000) {
                ...
                out.log(`"output-${txinfo.txid}:${index}" [shape="box", color="red", rank="sink", label=${label}, tooltip="",  weight=1]`);
            }
            else {
                const value = formatValue(output.value, 'BTC');
                const label = `<value: ${value}<br/>address: ${shortAddress(address)}>`;
                out.log(`"output-${txinfo.txid}:${index}" [shape="box", color="blue", rank="sink", label=${label}, tooltip="",  weight=1]`);
            }
            out.log(`"${txinfo.txid}" -> "output-${txinfo.txid}:${index}" [style=solid, tailport="s", headport="n", weight=1000000, arrowhead="none"]`);
        });
    });
    Object.values(transactions).forEach(txinfo => {
        txinfo.tx.ins.forEach((input, index) => {
            const ptxid = (0, bitcoin_1.hashToTxid)(input.hash);
            const ptxinfo = transactionMap[ptxid];
            if (!ptxinfo)
                return;
            out.log(`"output-${ptxid}:${input.index}" -> "input-${txinfo.txid}:${index}" [style="solid", tailport="s", headport="n", weight=1, arrowhead="none"]`);
        });
    });
    out.log('}');
}
