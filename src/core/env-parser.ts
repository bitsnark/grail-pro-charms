...
function parseEnv(
	...
	try {
		...
		logger.error(
			`Error parsing environment variable '${name}': ${error.message}`
		);
...
export const parse = {
	string: (name: string, defaultValue?: string): string =>
		parseEnv(name, parseString, defaultValue) as string,
	integer: (name: string, defaultValue?: number): number =>
		parseEnv(name, parseInteger, defaultValue) as number,
	bigint: (name: string, defaultValue?: bigint): bigint =>
		parseEnv(name, parseBigInt, defaultValue) as bigint,
	boolean: (name: string, defaultValue?: boolean): boolean =>
		parseEnv(name, parseBoolean, defaultValue) as boolean,
	number: (name: string, defaultValue?: number): number =>
		parseEnv(name, parseNumber, defaultValue) as number,
};
