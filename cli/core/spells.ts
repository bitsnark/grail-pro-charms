...
export async function createSpell(
	...
	const previousTransactions = await Promise.all(
		previousTxids.map(async txid => bitcoinClient.getTransactionHex(txid))
	);
...
export async function transmitSpell(
	...
	const signedCommitmentTxHex = await bitcoinClient.signTransaction(
		commitmentTxHex,
		undefined,
		'ALL|ANYONECANPAY'
	);
	...
	const commitmentTxid = await bitcoinClient.transmitTransaction(
	...
	const spellTxid =
		await bitcoinClient.transmitTransaction(spellTransactionHex);
